...
static def childDeviceCount() {
    // change to 6 for a 6 strip, else 5 for a 5 strip
    return 6
}
...
def parse(String description) {
    ...
    try {
        ...
        log.info "Exception parsing '${description}' : ${ex}"
    }
    if (cmd) {
        try {
            ...
            log.info "Exception processing zwave event ${cmd} from " +
                "${description}: ${ex}"
        }
    } else {
...
def installed() {
    ...
    // command(zwave.configurationV1.configurationSet(configurationValue:
    // [255], parameterNumber: 1, size: 1))
...
def pollNodes() {
    if (logEnable) {
        log.debug "Polling Powerstrip - ${device.label} node ${node}"
    }
    if (txtEnable) {
        log.info "Polling Powerstrip Nodes"
    }
    def cmds = []
    for (i in 1 .. childDeviceCount()) {
...
def pollNode(endpoint) {
    if (logEnable) {
        log.debug "Polling Powerstrip node ${endpoint}"
    }
    def cmds = []
...
private hubitat.zwave.Command encap(cmd, endpoint) {
    if (endpoint) {
...
def zwaveEvent(hubitat.zwave.commands.meterv3.MeterReport cmd, ep = null) {
    ...
    if (ep) {
        ...
        sendEvent(name: 'lastupdate', value: lastUpdated(now()), unit: "")
        sendEvent(result)
        (1 .. childDeviceCount()).each { endpoint ->
...
def zwaveEvent(hubitat.zwave.commands.multichannelv3.MultiChannelCmdEncap cmd) {
    ...
    def encapsulatedCommand = null
    try {
        cmd.encapsulatedCommand([0x30: 1, 0x31: 1])
    } catch (Exception ex) {
        log.error("Error getting encapsulated command ${cmd} : ${ex}")
    }
